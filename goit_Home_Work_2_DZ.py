...
# print (f'\nПовідомлення {message} \nзакодоване ключем {offset} \nбуде мати наступний вгляд : {encoded_message} ') # виводимо повідомлення .



# =======================  Завдання 14/Task_14  ==================

# +++++++++++++++++++++++  Опрацювання помилок за допомогою Конструкція (try  …  except )  +++++++++++++++++

# ************Синтаксис : **************************************
# try :
#     # *Код, який може згенерувати виняток*
# except *вказуємо тип помилки який треба опрацювати* : # Коли невказано тип помилки то спрацьову на всі .
                                                        # Для опрацювання різних винятків/помилок по різному , використовуємо кілька блоків *expect *помилка яку треба опрацювати* : тіло коду який треба виконати вразі отримання цього типу помилки  
#     # *Код для виконання у разі генерації винятку*

# також після блоків except : можна добавляти оператор *else: і *finally:
# деякий перелік винятків і як зними працювати  можна детальніше прочитати на https://acode.com.ua/exceptions-python/
...
# quantity = int ( input ("Enter the number of mailings: ") ) # *quantity - приймає значення введене з клавіатури приведене до типу *int.
#                                                             #  якщо введені дані не можна привести до типу *int викликче помилку *ValueError

# try:                                   # try(спробуй виконати) 
#                                        # оскільки в нас в коді є ділення то при введені в quantity == 0 виникне ситуація ділення на 0 , що є недопустимою операцією ,і викличе помилку програми *ZeroDivisionError.
#                                        # Щоб програма не вилетіла через некоректно введені дані нам потрібно опрацювати цю помилку .
#                                        # опрацьовуємо за добомогою (try  …  except )
#                                        # в тіло оператора *try: записуємо код який може викликати помилку , а після оператора except *назва помилки яку потрібно опрацювати* : виконуєм код який потрібно , вразі винекнення помилки
#                                        # в нашому випадку Видаємо на екран що виконано ділення на 0 .Заверши! якщо quantity  буде дорівнювати 0.
#                                        #  
#     chunk = pool // quantity           # Шукаємо цілу частину від ділення pool на quantity де "//"- повертає цілу частину від ділення (значення яке повертає - ціле число ,завжди типу *int) .
    
# except ZeroDivisionError :  # оператор *expect спрацьовує коли виникла поилка в тілі коду *try . В нашому випадку опрацьовуємо помилку *ZeroDivisionError (ділення на 0) *expect може бути без параметру (тоді спрацьовує на будь яку помилку ) або вказувати параметром (яка саме помилка має бути опрцьована) Так можна різні помилки опрацьовувати по різному. 
#                             # деякий перелік винятків  можна детальніше прочитати на https://acode.com.ua/exceptions-python/
#     print('Divide by zero completed!') # вивід на екран  *виконано ділення на 0 .Програму завершено !*


# =======================  Завдання 15/Task_15  ==================

# +++++++++++++++++++++++  ЗАВДАННЯ: ПІДСУМКОВЕ .Написання простого калькулятора  +++++++++++++++++
...
# може містити чотири значення, "+", "-", "*", "/" wait_for_number — прапорець, який вказує, 
...
def sum (a, b) : # власна функція ділення  . Можна скористатись вбудованою
...
def riz (a, b) : # власна функція ділення  . Можна скористатись вбудованою

    return round (a - b,3) #  повертаємо заокругленні значення оскільки будемо працювати з значеннями з пламаючою комою .
                           # як парцює round () -дивись в описі про додавання  * def sum (a, b) :

def dobutok (a, b) : #  власна функція ділення  .

    return round (a * b,3) #  повертаємо заокругленні значення оскільки будемо працювати з значеннями з пламаючою комою .
                           # як парцює round () -дивись в описі про додавання  * def sum (a, b) :

def dil (a, b) : # власна функція ділення  .
...
def perevirka_num ( operand ) : # власна  функція яка перевіряє чи введене значення operand є числом .Якщо ні повідомляє що символ який введено не є числом.
...
def perevirka_znak () :  # власна  функція яка перевіряє чи введено математичний знак +,-,*,/ або = . Якщо введено математичний знак повертає його ,
                         # якщо будь що інше повідомляє що введено не знак # і просить ввести повторно математичний знак .Так до тих пір , поки не введуть математичний знак 
                         

    print(f'Для завершення обрахунку = ') # виводить повідомлення *Для завершення обрахунку = *  ( якщо ввести " = " то вводити числа більше не потібно , а потрібно обрахувати все що було введено до того як ввели "=" )

    operator = input ("Введіть знак : ")  # *operator присвоюємо значення ведене з клавіатури. Користувачу виводить  повідомлення "Введіть знак : " і очікує на введене значення з клавіатури.
    
    if operator =="+" or operator =="*" or operator =="-" or operator =="/" or operator =="=" : # Умова де перевіряємо введене з клавіатури значення на відповідність матиматичному знаку перечисених в умові через оператор *or (або).

        return operator # якщо умова  виконується Тобто введений символ відповідає хочаб одному з перечслених. Повертаємо значення  математичний знаку з функції.
    
    else :                                       # інакше виводимо виводмо повідомлення на екран.

        print(f"{operator} - не є матиматичним знако +,-,*,/ . Завершити введення") # вивід на екран "{operator} - не є знак +,-,*,/" , де {operator} - смивол який ввели.

def  perevirka_end (operator , count ) : # власна  функція яка перевіряє чи завершився вввід всіх даних і чи виконується умова що для обрахунку потрібно хочаб 2 числа і один математичний знак.
                                     # функція отримує два параметри . зміну operator типу *str (очікувано математичний знак )
                                    #  і зміну count типу *int (лічильник який відображає кількість правильно введених значень , потрібний для перерки чи введено достатнь значень для здійснення хочаб одного обрахунку.)
    
    condition = True # Умова для завершення циклу *while condition :*. *сondition присвоюємо початкове значення *True

    while condition : # Цикл while з умовою *condition буде виконуватись доти доки *condition == True. В ньому і реалізуємо код виконання вимого функці.

        if operator == "=" or count <= 1: # Умова перевірємо передані у функцію значення . Якщо умова виконується виконуємо тіло умови.
            
            if count == 0 : # Якщо лічильик рівний 0 , це означає що після введення першого чила ввели знак = а не знак матиматичної дії .
                             
                print(f"\nВи невели математичного знаку для виконання обрахунку \nВи бажаєте завершити ? \nТак , натисніть '=' \n") # Виводимо повідомлення де попереджаємо що "Ви невели числа для виконання обрахунку" 
                                                                                                                      # Запитуємо "Ви бажаєте завершити ?" Для завершення пропонуємо ввести "="

            count += 1 # Збільшуємо лічильник на 1 щоб більше не виводити попереднє повідомлення  .

            operator = input ("""введіть "="" для завершення """) # Пропонуємо ввести "=" для завершення 
            
            if operator == "=" :      #   Перевіряємо ще раз чи ввели "=" . Якщо operator == "="  Значить це не випадкове натискання "=" а свідомий вибір корстувача і є потребе завершити прграму і виконати обрахунок введених даних. Якщо їх буде недостатньо повідомити це користувачу. 
                    print("Вихід")    #   виводимо на екрані "Вихід"
                    condition = False #   *condition присвоюємо  *False  для завершення   циклу *while condition : 
                    return condition  #   Повертаємо *сondition з значенням *False
        

        operator = perevirka_znak () # Пропонуємо ще раз ввести математичний знак чи "=", якщо буде введено будь що інше ще 2 рази програма завершиться і видась резултат.
        return operator              # повертаємо значення введене користувачем для подпльшої обробки . Якщо буде введено 
                
        
   
condition = True  # умова для переривання циклу *while condition
count = 0   # лічильник для підрахунку кількість правильно введених послідовностей .За ним перевіряємо чи користувач ввів як мінімум 2 числа і один матиматичний знак . Інакше повідомляємо що введено не достатньо даних і чи він хоче насправді завершити роботу . якщо так то натискає ще раз "="" і програма завершується.

while condition : # основний цик в якому будемо просити користувача ввести дані для обрахунку . # Дані водить в наступному порядку . число , матиматичний знак , число ,матиматичний знак і так далі . Для кожного вводу будемо перевіряти чи вірно введені дані . як описано в умові задачі. 
   
   operand = input ( "Введіть число : " ) # *operand присвоюємо значення введені з клавіатури.  Переше введене значення .
   
   if perevirka_num ( operand ) != None : # функція отримує введене значення *operand типу *str , приводить його о типу *float і повертає.
                                          # якщо введене значення не вдаться привести до *float .То виводить повідомлення " {operand} не є числом !!! "
                                          # пройде все тіло коду де жодна умова невиконається і поверненться на початок де знову попросить вести число і так то тих пір поки корстувач не введе число. 
        
        list.append( perevirka_num (operand) ) # якщо умова *if perevirka_num ( operand ) != None* виконається значить користувач вів число . Це число приведене до типу *float поверне фінкція *perevirka_num (operand)
                                               # введене число в *operand додаємо в список *list за допомогою метода *спиок.append(*значення яке треба добавити в список*)

        

        while condition :  # вкладений цикл *while з основною умовою  завершення *condition а також додаткова умова якщо виконується то вклаений цикл завершується операторм *break
                           # в цьому циклі будемо запитувати користувача вести математичний знак і перевіряти його на правильність введення . за допомогою вуклику функції *perevirka_znak()
                           # *perevirka_znak() повідомляє користувча що для завершення обрахунку потрібно ввести знак "=" . перевіряє чи введений знак є математичним . і якщо так повертає нам введений знак типу *str
            
            operator = perevirka_znak() # в *operator присвоюєм значення математичного знаку .Поверне або "+","-","*","/" або "="

            if operator == "+" or operator == "*" or operator == "-" or operator == "/"  : # Перевіряємо *operator на відповідний математичний знак , якщо ні - значить operator == "="
                
                list.append(operator) #  якщо умова виконується додаєм знак в список *list за допомогою методу *список.append(*значення)
                count += 1 # збільшуємо лічильник на 1 .на мо
                #print(list)
                break    # якщо математичий знак введений - перериваємо вбудований цикл while condition :  . і повертаємось на початок першого кроку циклу *while condition : де просимо знову ввести число .
          
              
            condition = perevirka_end (operator, count ) # якщо умова  *if operator == "+" or operator == "*" or operator == "-" or operator == "/"  :*  не виконалась значить  *operator == "=".
                                                         # функія perevirka_end (operator, count ) отримує поточні  значення  *operator і *count . Пропонує чи ведено достатньо значень для обрахунку .і пропонє ввести їх , якщо все таки користувач підтвердить намір завершити то введе ще раз "=" і функція поверне *False
                
                                                         # в *condition присвоюєм значення яке поверне * фукція perevirka_end (operator, count ) . якщо condition == *False значить є потреба завершити введення чисел і знаків і почати обрахунок. condition == *False - Завершить виконання двох циклів while condition : оскльки в них однакова умова для звершення *condition == False
            
           
          
 #   Алгоритм обрахунку : 
 #  Оскільки в списку буде записано строго [ Число , математичний знак , Число ,і так далі ] . 
 # Тобто всі чила будуть мати непарний індекс, а математичні заки парний . Приклад сиску *list =[5,"+", 10, "*", 2, "/", 4 ,.... ]
 # Для обрахунку будемо брати спочатку два перших числа попорядку з списку і перевіряти знак між ними . В *result записуємо значення обрахунку , 
 # далі беремо значення  *result і наступне число і також обраховуємо згідно знаку між ними , Отримане значення перезаписуємо в *result,
 # і так до кінця списку.  
              
if len (list) > 2 :  # перевіряємо чи довжина нашого списку більша за 2 . якщо більше виконуємо покрокові математичні дії строго з ліва на право. 
    
    result = list [0] # Оголошуємо початкове значення зміної *result(зміна буде містити  результат обрахунку значень вказаних в сиску *list ) . Перший елемент з списку *list (список містить числа  і дії для обрахунку). Це завжди перша введена цифра.
    index = 0        # індекс списку .
        
    for i in list:   #  Цикл  for в якому будемо рухатись по сформованому списку і обраховувати результат введених даних користувачем. Зміна *i буде набувати елемент з списку на кожному кроці циклу.
                     #  на першому кроці *result буде містити значення нульового елемента списку *list. Це завжди перше число з списку. 
                     #  якщо *i буде дорівнювати числу то жодна з умов наведених нижче не виконається .На наступному кроці *i набуде значення матиматичної дії з списку .
                     #  В залежності від знаку виконається одна з умов перечислених нижче . Тоді згідно тіла умови виконається відповідна функція і поверне в *result нове значення. а *index збільиться на 2 . 
         
         
        
        if i == "+" :  # Перевірка чи додавання . якщо елемент списку i == "+" будемо додавати  *result до елементу з списку індекс якого кратний 2 .
            index += 2 # присвоюєм index нове значення , до попереднього дадаємо 2 
            result = sum ( result , list [index] ) # в *result записуємо нове значення повернуте функцією , *sum ( result , list [index] ) , де *result - попереднє заначення ,list [index] - наступне число після матиматичного знаку , елемент списку *index якого картний 2  
            
            
        if i == "*" :  # Перевірка чи множеня .
            index += 2 # присвоюєм index нове значення , до попереднього дадаємо 2 
            result = dobutok ( result, list [index] ) # в *result записуємо нове значення повернуте функцією , *dobutok ( result , list [index] ) , де *result - попереднє заначення ,list [index] - наступне число після матиматичного знаку , елемент списку *index якого картний 2
            
        
        if i == "-":   # Перевірка чи віднімання  .
            index += 2 # присвоюєм index нове значення , до попереднього дадаємо 2 
            result = riz ( result, list [index] )   # в *result записуємо нове значення повернуте функцією , *riz ( result , list [index] ) , де *result - попереднє заначення ,list [index] - наступне число після матиматичного знаку , елемент списку *index якого картний 2
            
            

        if i == "/" :  # Перевірка чи ділення .
            index += 2 # присвоюєм index нове значення , до попереднього дадаємо 2 
            result = dil ( result, list [index ] )  # в *result записуємо нове значення повернуте функцією , *dil ( result , list [index] ) , де *result - попереднє заначення ,list [index] - наступне число після матиматичного знаку , елемент списку *index якого картний 2
            
        
    print(f"\n {list} = {result}") # виводимо на екран список введених користувачем всіх чисел і дії які потрібно виконати і результат який отримаємо .

else:                              # інакше стосується умови *if len (list) > 2 :*  список буде містити 2 і менше елементи , Тоді виводимо на екра  що замало значень для обрахунку і завершуємо роботу програми.                 
        print(f"Ви ввели недостатньо значень для обрахунку \nПрограма завершилась!!!") # Виввід на екран повідомлення. "Ви ввели недостатньо значень для обрахунку \nПрограма завершилась!!!" , 
                                                                                       # де *\n - вивід знаступного рядка .Все що після *\n буде виведено в новому ряку.
